...
imgslist,labslist=ListFromCSV(LearnDataPATHcsv,picksize=10000)
Valimgslist,Vallabslist=ListFromCSV(LearnDataPATHcsv,picksize=100)

if LOADMODEL:
    model=tf.keras.models.load_model(r'/home/vjekod/Desktop/CULane_seg_label_generate/Results/epoch-0201-val_loss-0.0354-val_acc-0.0000.hdf5')
else:
...
model_checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(
    filepath='Results/'+"epoch-{epoch:04d}-val_loss-{val_loss:.4f}-val_acc-{val_accuracy:.4f}.hdf5",
...
for ix in range(0,20):
    ...
    for i in range(0,len(a)-1,1):
        ...
        acclist+=(h.history['accuracy'])
        valacclist+=(h.history['val_accuracy'])
        loss=vallosslist[-1]
        acc=vallosslist[-1]
        del X,Y
        name = 'Results/rerunafter219400x400_' + str(ix) + "acc" + str(acc) + "ep_" + str(curepoch)
